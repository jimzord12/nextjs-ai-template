---
name: do-issue
description: Implements a single `ready-for-agent` issue end-to-end, following the specified method (TDD or chore) and orchestrating subagents for the entire implementation.
---

# Agent Skill: do-issue

Implements a single `ready-for-agent` issue end-to-end, following the specified method (TDD or chore) and orchestrating subagents for the entire implementation.

The issues are located in `.scratch/<feature-slug>/issues` and have a `Status` field.

Each issue must have at the top:
 - a `Method:` (e.g. `tdd`, `chore`) that dictates the implementation workflow to follow.
 - a `Status:` to indicate the current state of the issue (e.g. `ready-for-agent`, `ready-for-human`).
 - a `Complexity:` a number from 1-5 to help you gauge the effort required.

**CRITICAL**: if the selected issue does not have even one of these: `Method:`, `Status:`, or `Complexity:` fields, it cannot be processed and the opearation must be aborted with a message to the user indicating which field(s) are missing.

## Issue Selection

> Blocked until CLI Tool is created to list and select issues. For now, the user must specify the issue file path directly when invoking this skill, e.g. `/do-issue .scratch/authentication/issues/issue-123.md`.
